...
const solution4 = (number: number[]) => {
  let answer = 0;

  const setNumber = (curr: number[], start: number) => {
    console.log("curr", curr, "start", start);
    if (curr.length === 3) {
      return (answer +=
        curr.reduce((acc: number, cur: number) => acc + cur, 0) === 0 ? 1 : 0);
    }
